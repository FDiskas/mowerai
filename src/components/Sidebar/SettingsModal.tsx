        ...
        <div className="modal-backdrop" onClick={props.onClose}>
            <div className="modal-content border border-slate-700/50 max-w-md" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h2 className="text-2xl font-black text-white uppercase tracking-tight">Simulation Settings</h2>
                        <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1">Configure robot parameters</p>
                    </div>
                ...
                <div className="space-y-8 bg-slate-950/50 p-8 rounded-[2rem] border border-slate-800 shadow-inner">
                    <div className="space-y-6">
                        <Slider label="Battery Capacity" value={props.maxBattery} min={10} max={200} step={10} onChange={props.setMaxBattery} disabled={props.isRunning} suffix="%" />
                        <Slider label="Energy Consumption (movement)" value={props.drainMove} min={0.01} max={1.0} step={0.01} onChange={props.setDrainMove} disabled={props.isRunning} />
                        <Slider label="Energy Consumption (turn)" value={props.drainTurn} min={0} max={2.0} step={0.1} onChange={props.setDrainTurn} disabled={props.isRunning} />
                        <Slider label="Simulation Speed" value={props.speed} min={10} max={135} step={5} onChange={props.setSpeed} disabled={props.isRunning} />
                        
                        <div className="space-y-3 pt-2">
                            <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest block text-left">Mowing Direction</label>
