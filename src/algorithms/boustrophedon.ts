...
export const getBoustrophedonMove = (
  ...
  if (!state.cellData) {
    ...
    state.cellData = {
      ...
      cellScanDir: 1,
      orientation:
        state.orientation ?? computeSweepOrientation(curGrid, cellTypes),
      phase: "edge",
    };
  }
  ...
  if (cellData.phase === "edge") {
    ...
    const toBorder = findPathToTarget(pos, curGrid, prevDir, (p) =>
      isBorderGrass(p.x, p.y),
    );
  ...
  const closest = findPathToTarget(
    pos,
    curGrid,
    prevDir,
